...
def log(funcao):
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            with open(ROOT_PATH/NOVO_DIRETORIO/'log.txt', 'a', encoding='utf-8', newline='') as arquivo:
                arquivo.write(f"[{timestamp}] Função: '{funcao.__name__}' executada com {args} {kwargs} Retornou {resultado} \n")
        except IOError as e:
...
def extrato(clientes):
    ...
    if cliente:
        ...
        transacoes_realizadas = len(
            [
                transacao
                for transacao in cc.historico.transacoes
                if transacao["tipo"] in ["Saque", "Deposito"]
            ]
        )

        print(f"\n\n{'Extrato'.center(40, '=')}")
        print(
            f"Cliente: {cliente.nome}\nTransações Restantes: {cc.limite_transacoes - transacoes_realizadas}\n\n"
        )
